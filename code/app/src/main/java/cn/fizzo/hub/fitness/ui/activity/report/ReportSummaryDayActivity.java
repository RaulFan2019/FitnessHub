...
public class ReportSummaryDayActivity extends BaseActivity {
    ...
    private GetReportSummaryRE mGetSummaryDayRe;//统计结果
...
//        Log.v(TAG, "keyCode:" + keyCode);
    ...
    protected void myHandleMsg(Message msg) {
        switch (msg.what) {
            case MSG_POST_ERROR:
                Toast.makeText(ReportSummaryDayActivity.this, (String) msg.obj,Toast.LENGTH_LONG).show();
                finish();
                break;
            case MSG_POST_OK:
                vLoading.loadFinish();
    ...
    private void postGetSummaryDayData() {
        x.task().post(new Runnable() {
            @Override
            public void run() {
                int storeId = SPDataConsole.getStoreId(ReportSummaryDayActivity.this);
                String day = TimeU.getDayByDelay(1);
                RequestParams params = RequestParamsBuilder.buildGetReportSummaryDayRP(ReportSummaryDayActivity.this,
                ...
                mCancelable = x.http().post(params, new Callback.CommonCallback<BaseRE>() {
                    ...
                    public void onSuccess(BaseRE result) {
                        if (result.errorcode == BaseResponseParser.ERROR_CODE_NONE) {
                            mGetSummaryDayRe = JSON.parseObject(result.result, GetReportSummaryRE.class);
    ...
    private void updateSummaryView() {
        ...
        tvTitle.setText(getResources().getString(R.string.report_title_yesterday) + "（" + mGetSummaryDayRe.total_movercount + "）");
