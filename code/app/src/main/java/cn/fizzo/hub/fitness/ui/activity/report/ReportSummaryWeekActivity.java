...
public class ReportSummaryWeekActivity extends BaseActivity {
    ...
    protected int getLayoutId() {
        return R.layout.activity_report_summary_week;
    }
...
//        Log.v(TAG, "keyCode:" + keyCode);
    ...
    protected void myHandleMsg(Message msg) {
        switch (msg.what) {
            case MSG_POST_ERROR:
                Toast.makeText(ReportSummaryWeekActivity.this, (String) msg.obj,Toast.LENGTH_LONG).show();
                finish();
                break;
            case MSG_POST_OK:
                vLoading.loadFinish();
    ...
    protected void initViews() {
        //列表
        mAdapter = new ReportSummaryListAdapter(ReportSummaryWeekActivity.this, listMover);
        list.setAdapter(mAdapter);
    }

    @Override
    protected void doMyCreate() {
        postGetSummaryWeekData();
    }
     ...
     * 获取报告
     */
    private void postGetSummaryWeekData() {
        x.task().post(new Runnable() {
            @Override
            public void run() {
                int storeId = SPDataConsole.getStoreId(ReportSummaryWeekActivity.this);
                String day = TimeU.getWeekByDelay(1);
                RequestParams params = RequestParamsBuilder.buildGetReportSummaryWeekRP(ReportSummaryWeekActivity.this,
                        UrlConfig.URL_GET_REPORT_SUMMARY_WEEK, storeId, day);
    ...
    private void updateSummaryView() {
        ...
        tvTitle.setText(getResources().getString(R.string.report_title_last_week) + "（" + mGetSummaryRe.total_movercount + "）");
