...
//		Log.v(TAG,"mTabWidget.getTabCount():" + mTabWidget.getTabCount());
		//在adapter设置成功后，让每个tab都能根据焦点事件控制TAB
		for (int i = 0; i < mTabWidget.getTabCount() ; i++ ){
			final int pos = i;
			mTabWidget.getChildTabViewAt(i).setOnFocusChangeListener(new OnFocusChangeListener() {
				@Override
				public void onFocusChange(View v, boolean hasFocus) {
					Log.v(TAG,"pos:" + pos + ",hasFocus:" + hasFocus + ",lastPos:" + lastPos);
					if (hasFocus){
						if (lastPos != pos && mOnTabSelectListener != null){
							lastPos = pos;
							TabWidget tw = getTabWidget();
							View titleWidget = tw.getChildTabViewAt(pos);
							mOnTabSelectListener.onTabSelect(OpenTabHost.this, titleWidget, pos);
						}
...
//							TabWidget tw = getTabWidget();
//							View titleWidget = tw.getChildTabViewAt(pos);
//							cb.onTabSelect(OpenTabHost.this, titleWidget, pos);
//						}
//					}
				}
			});
